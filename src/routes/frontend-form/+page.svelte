<script lang="ts">
  ...
  const workRightOptions = [
    ...
    { value: "holiday_visa", label: "I have a holiday temporary work visa" },
    {
      value: "regional_visa",
      label:
        "I have a temporary visa with restrictions on location (e.g. Skilled Regional 491)",
    },
    {
      value: "protection_visa",
      label: "I have a temporary protection/safe haven work visa",
    },
    {
      value: "doctoral_visa",
      label: "I have a temporary visa with no restrictions (e.g. Doctoral)",
    },
    {
      value: "hour_restricted_visa",
      label: "I have a temporary visa with restricted hours (e.g. Student)",
    },
    {
      value: "industry_restricted_visa",
      label:
        "I have a temporary visa with restricted industry (e.g. activity visa 408)",
    },
    {
  ...
  async function loadUploadedResumes() {
    ...
    try {
      ...
      availableResumeFiles = entries
        .map((e) => e.filename)
        .filter(
          (n): n is string => typeof n === "string" && isSupportedResumeFile(n),
        );
    } catch (error) {
  ...
  async function handleResumeUpload(event: Event) {
    ...
    if (file) {
      ...
      const userEmail = (formData.email || "").trim();
      if (!userEmail) {
        alert("Please add your Email in Profile & Contact first.");
        if (target) target.value = "";
        return;
      }
      try {
        ...
        const extractRes = await fetch(
          ...
          { method: "POST", body: extractFormData },
        );
  ...
  async function syncSelectedResumeToCorpus() {
    ...
    try {
      ...
      if (!entries.some((e) => e.filename === formData.resumeFileName))
...
<div
  class="min-h-screen bg-gradient-to-br from-base-200 via-base-100 to-base-300 pt-4 pb-12 px-2 sm:px-4 lg:px-6 relative overflow-hidden"
>
  <div
    class="absolute top-0 -left-4 w-96 h-96 bg-primary rounded-full mix-blend-multiply filter blur-3xl opacity-[0.02] animate-blob"
  ></div>
  <div
    class="absolute top-0 -right-4 w-96 h-96 bg-secondary rounded-full mix-blend-multiply filter blur-3xl opacity-[0.02] animate-blob animation-delay-2000"
  ></div>
  ...
  <div class="max-w-4xl mx-auto relative z-10">
    <div class="mb-4 flex justify-between items-center">
      <div>
        <h1
          class="text-2xl font-bold text-base-content tracking-tight flex items-center gap-2"
        >
          ⚙️ Configuration
        </h1>
        <p class="text-base-content/60 text-xs mt-0.5">
          Customize your automated job application preferences
        </p>
      </div>
    </div>
    ...
    >
      <!-- Profile & Contact -->
      <div
        class="card bg-base-100/90 backdrop-blur-xl border border-white/20 shadow-md hover:shadow-lg transition-all duration-300 group"
      >
        <div class="card-body p-3">
          <h2
            class="card-title text-base mb-0.5 flex items-center gap-1.5 text-base-content/90 font-bold"
          >
            👤 Profile & Contact
          </h2>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-1">
            <div class="form-control w-full">
              ...
              <input
                ...
                class="input input-bordered input-sm w-full {formErrors.fullName
                  ? 'input-error'
                  : ''}"
              />
              {#if formErrors.fullName}<span
                  class="text-error text-sm mt-1 ml-1"
            ...
            <div class="form-control w-full">
              ...
              <input
                ...
                class="input input-bordered input-sm w-full {formErrors.email
                  ? 'input-error'
                  : ''}"
              />
              {#if formErrors.email}<span class="text-error text-sm mt-1 ml-1"
            ...
            <div class="form-control w-full">
              ...
              <input
                ...
                class="input input-bordered input-sm w-full {formErrors.phone
                  ? 'input-error'
                  : ''}"
              />
              {#if formErrors.phone}<span class="text-error text-sm mt-1 ml-1"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-1">
            <div class="form-control w-full">
              ...
              <input
                ...
                class="input input-bordered input-sm w-full {formErrors.keywords
                  ? 'input-error'
                  : ''}"
              />
              {#if formErrors.keywords}<span
                  class="text-error text-sm mt-1 ml-1"
                  >{formErrors.keywords}</span
                >{/if}
            </div>
            <div class="form-control w-full">
            ...
            <div class="form-control w-full">
              ...
              <select
                ...
                class="select select-bordered select-sm w-full text-sm"
              >
                <option value="any">Any</option><option value="entry"
                  >Entry</option
                ><option value="mid">Mid</option><option value="senior"
                  >Senior</option
                >
              </select>
            </div>
            <div class="form-control w-full">
              ...
              <select
                ...
                class="select select-bordered select-sm w-full text-sm"
              >
                {#each industries as ind}<option value={ind.value}
                    >{ind.label}</option
                  >{/each}
              </select>
            </div>
      ...
      <div
        class="card bg-base-100/90 backdrop-blur-xl border border-white/20 shadow-md p-3"
      >
        <h2
          class="card-title text-base mb-0.5 flex items-center gap-1.5 font-bold"
        >
          🏛️ Work Rights
        </h2>
        <div class="form-control w-full">
          ...
          <select
            ...
            class="select select-bordered select-sm w-full text-sm mt-1"
          >
            {#each workRightOptions as opt}<option value={opt.value}
                >{opt.label}</option
              >{/each}
          </select>
        </div>
      ...
      >
        ...
        <div class="space-y-1">
          <label class="label cursor-pointer justify-start gap-2 py-0">
            ...
            />
            <span class="label-text font-semibold text-xs"
              >Rewrite resume for each Job?</span
            >
          </label>
          <div class="form-control w-full">
            <select
              bind:value={formData.botMode}
              class="select select-bordered select-sm w-full text-sm"
            >
      ...
      <div class="space-y-3">
        <div
          class="card bg-base-100/90 backdrop-blur-xl border border-white/20 shadow-md"
        >
          <div class="card-body p-3">
            <h2
              class="card-title text-base mb-0.5 flex items-center gap-1.5 text-base-content/90 font-bold"
            ...
            <div class="grid grid-cols-1 gap-1.5">
              <div class="form-control w-full">
                ...
                >
                  ...
                  {#each availableResumeFiles as name}<option value={name}
                      >{name}</option
                    >{/each}
                </select>
              </div>
            ...
            {#if uploadValidationMessage}<div
                class="text-sm mt-2 opacity-70 font-medium"
              >
                {uploadValidationMessage}
              </div>{/if}
          </div>
            ...
            <div class="grid grid-cols-1 gap-1.5">
              <div class="form-control w-full">
                <label class="label py-0" for="excluded-companies-input">
                  <span class="label-text font-semibold text-[10px] opacity-70"
      ...
      <div>
        <div class="card bg-warning/5 border border-warning/10 shadow-sm">
          <div
            class="card-body p-1.5 flex flex-row items-center justify-between gap-3"
          >
            <div class="flex items-center gap-3">
              <span class="text-warning text-lg">⚠️</span>
              <div class="flex flex-col">
                <span class="text-[12px] font-bold text-warning"
                  >Legal Disclaimer</span
                >
                <span class="text-[10px] opacity-80 leading-tight font-medium"
                  >By using this automation tool, you assume all responsibility
                  for TOS compliance.</span
                >
              </div>
            </div>
            <label
              class="label cursor-pointer gap-2 py-0 border-l border-warning/10 pl-3"
            >
              <input
                id="acceptTerms-checkbox"
                type="checkbox"
                bind:checked={formData.acceptTerms}
                class="checkbox checkbox-warning checkbox-sm {formErrors.acceptTerms
                  ? 'checkbox-error'
                  : ''}"
              />
              <span
                class="label-text font-bold text-[10px] text-warning uppercase tracking-wider"
                >I understand and accept</span
              >
            </label>
          </div>
          {#if formErrors.acceptTerms}<div
              class="px-2 pb-1 text-error text-[10px] text-center"
            >
      ...
      <div
        class="sticky bottom-0 z-40 bg-base-100/95 backdrop-blur-md border-t border-base-200/50 p-3 -mx-4 mt-6 shadow-[0_-4px_12px_rgba(0,0,0,0.1)]"
      >
        <div class="flex justify-center items-center gap-4">
          <button
            type="submit"
            class="btn btn-primary btn-sm px-8 shadow-lg text-white border-0 bg-gradient-to-r from-primary to-secondary hover:scale-105 transition-all duration-300"
          ...
          <button
            type="button"
            class="btn btn-outline btn-sm px-8 hover:bg-base-200 transition-colors"
            onclick={resetForm}
          >
            🔄 Reset Form
          </button>
