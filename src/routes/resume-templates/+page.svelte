<script lang="ts">
	...
	import { buildResumeTemplateCategories } from '$lib/data/resumeTemplates';
	import { loadResumeTemplatesCatalog } from '$lib/resume-templates-catalog';
	import TemplateCard from '$lib/components/TemplateCard.svelte';

	let selectedCategory = 'all';
	let templates: HybridTemplate[] = [];
	let loadError = '';
	let catalogLoaded = false;

	$: categories = buildResumeTemplateCategories(templates);
	...
	onMount(() => {
		(async () => {
			...
			try {
				...
				templates = [];
				loadError =
					e instanceof Error ? e.message : 'Could not load resume templates. Please try again later.';
			} finally {
				catalogLoaded = true;
			}
		})();
	...
	<div class="max-w-7xl mx-auto space-y-12">
		...
		<div class="text-center space-y-4">
			...
			<p class="text-lg text-base-content/70 max-w-2xl mx-auto">
				Stand out from the crowd with our professionally designed resume templates. Choose a template
				that fits your profession and download it to get started.
			</p>
			{#if loadError}
				<p class="text-sm text-base-content/70 max-w-2xl mx-auto">{loadError}</p>
			{/if}
		</div>
		...
		<div class="flex flex-col lg:flex-row gap-8">
							...
							<li>
								...
								>
									...
									<span class="badge badge-sm"
										>{category.id === 'all'
											? templates.length
											: templates.filter((t) => t.category === category.id).length}</span
									>
								</button>
							</li>
						{/each}
			...
			<div class="flex-grow">
				<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
					...
					{#if catalogLoaded && !loadError && filteredTemplates.length === 0}
						<div
							class="col-span-full py-12 text-center text-base-content/50 bg-base-100 rounded-xl shadow-sm"
						>
							<svg
