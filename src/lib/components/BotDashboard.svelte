<script>
  ...
  $: displayName = (storeBot?.name || botName || "Bot")
    .replace(/_bot$/, "")
    .replace(/_/g, " ");
  ...
  $: isApplyBot =
    displayName.toLowerCase().includes("apply") || displayAppliedJobs > 0;
  $: progressValue = isApplyBot ? displayAppliedJobs : displayJobsProcessed;
  $: progressTarget = isApplyBot
    ? displayTotalJobs || 1
    : displayExtractLimit || displayTotalJobs || 0;
  $: progressPercent =
    progressTarget > 0
  ...
  $: jobsLink = platform.includes("linkedin")
    ? "/linkedin-job-tracker"
    : platform.includes("seek")
      ? "/seek-job-tracker"
  ...
  $: completionTitle = isApplyBot
    ...
    : "Extraction Complete!";
  $: completionActionText = isApplyBot
...
<div class="card bg-base-200 shadow-xl border border-base-300 w-full">
  {#if storeBot?.attentionNeeded}
    <div
      class="bg-warning text-warning-content px-4 py-3 flex items-center justify-between border-b border-warning/20 animate-pulse"
    >
      <div class="flex items-center gap-3">
        ...
        <span class="font-bold text-sm uppercase"
          >Attention Required: Please login in the browser window</span
        >
      </div>
    </div>
  {/if}
    ...
    <div class="mt-3">
      <div class="flex justify-between items-baseline mb-1">
        ...
        <span class="font-bold text-2xl tabular-nums">
          {progressValue}<span class="text-base-content/40 text-lg"
            >/{progressTarget}</span
          >
        </span>
      </div>
      ...
      ></progress>
      <div class="text-right text-xs text-base-content/50 mt-0.5">
        {progressPercent}%
      </div>
    </div>
  </div>
  ...
  {#if isExpanded}
    <div class="border-t border-base-300">
      <div
        role="tablist"
        class="tabs tabs-bordered px-4 flex items-center justify-between"
      >
        <div class="flex">
          <button role="tab" class="tab tab-active"> Logs </button>
        </div>
    ...
    <div class="px-4 pb-4 pt-2">
      ...
      >
        {#if displayLogs.length === 0}
          <div class="p-4 text-base-content/40 italic">
            Waiting for events...
          </div>
        {:else}
          {#each displayLogs as log}
            <div class="px-3 py-0.5 flex gap-2 hover:bg-base-100/30">
              <span class="text-base-content/40 flex-shrink-0 select-none"
