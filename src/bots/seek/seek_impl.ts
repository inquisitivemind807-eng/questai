...
export async function* detectApplyType(ctx: WorkflowContext): AsyncGenerator<string, void, unknown> {
  try {
    ...
    if (result.hasQuickApply) {
      ...
      // Highlight the Quick Apply button for visual feedback
      try {
        await highlightSelector(ctx.driver, '[data-automation="job-detail-apply"]', '#00ff00', false).catch(() => {});
        // Also try generic button/a with Quick Apply text
        const qaBtn = await ctx.driver.findElement({
          xpath: "//*[(self::button or self::a or @role='button') and contains(translate(text(), 'QUICK APPLY', 'quick apply'), 'quick apply')]"
        }).catch(() => null);
        if (qaBtn) await highlightElement(ctx.driver, qaBtn, '#00ff00').catch(() => {});
      } catch (_h) { /* highlight not critical */ }
      if (ctx.overlay) {
        try {
      ...
      // Highlight the regular Apply button
      try {
        await highlightSelector(ctx.driver, '[data-automation="job-detail-apply"]', '#ff8800', false).catch(() => {});
        const regBtn = await ctx.driver.findElement({
          xpath: "//*[(self::button or self::a or @role='button') and (normalize-space(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'))='apply' or normalize-space(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'))='apply now')]"
        }).catch(() => null);
        if (regBtn) await highlightElement(ctx.driver, regBtn, '#ff8800').catch(() => {});
      } catch (_h) { /* highlight not critical */ }
      if (ctx.overlay) {
